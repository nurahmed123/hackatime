PetiteVue.createApp({
    ...
    vibrantColorsEnabled:
        JSON.parse(localStorage.getItem('wakapi_vibrant_colors')) || false,
    labels: {},
    get tzOptions() {
        return [
            defaultTzOption,
    ...
    confirmWakatimeImport() {
        if (confirm('Are you sure? The import can not be undone.')) {
            ...
            document.getElementById('use_legacy_importer').value = document
                .getElementById('use_legacy_importer_tmp')
                .checked.toString()
