...
function draw(subselection) {
    ...
    function shouldUpdate(index) {
        return (
            !subselection ||
            (subselection.includes(index) &&
                data[index].length >= showTopN[index])
        )
    }
            ...
            ? new Chart(projectsCanvas.getContext('2d'), {
                  ...
                  data: {
                      datasets: [
                          {
                              data: wakapiData.projects
                                  .slice(
                                      ...
                                      Math.min(
                                          showTopN[0],
                                          wakapiData.projects.length
                                      )
                                  )
                                  .map((p) => parseInt(p.total)),
                              backgroundColor: wakapiData.projects.map(
                                  (p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                              ...
                              hoverBackgroundColor: wakapiData.projects.map(
                                  (p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                                              ? getRandomColor(p.key)
                      ...
                      labels: wakapiData.projects
                          .slice(
                              0,
                              Math.min(showTopN[0], wakapiData.projects.length)
                          )
                          .map((p) => p.key),
                  },
                  options: {
                      ...
                      onClick: (event, data) => {
                          ...
                          url.searchParams.set(
                              'project',
                              name === 'unknown' ? '-' : name
                          ) // will break if the project is actually named "unknown"
                      ...
                      datasets: [
                          {
                              data: wakapiData.operatingSystems
                                  .slice(
                                      ...
                                      Math.min(
                                          showTopN[1],
                                          wakapiData.operatingSystems.length
                                      )
                                  )
                                  .map((p) => parseInt(p.total)),
                              backgroundColor: wakapiData.operatingSystems.map(
                                  (p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                                              ? osColors[p.key.toLowerCase()] ||
                                                    getRandomColor(p.key)
                                              : getColor(p.key, i)
                                      )
                                      return `rgba(${c.r}, ${c.g}, ${c.b}, 1)`
                              ...
                              hoverBackgroundColor:
                                  wakapiData.operatingSystems.map((p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                                              ? osColors[p.key.toLowerCase()] ||
                                                    getRandomColor(p.key)
            ...
            ? new Chart(editorsCanvas.getContext('2d'), {
                  ...
                  data: {
                      datasets: [
                          {
                              data: wakapiData.editors
                                  .slice(
                                      ...
                                      Math.min(
                                          showTopN[2],
                                          wakapiData.editors.length
                                      )
                                  )
                                  .map((p) => parseInt(p.total)),
                              backgroundColor: wakapiData.editors.map(
                                  (p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                                              ? editorColors[
                              ...
                              hoverBackgroundColor: wakapiData.editors.map(
                                  (p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                                              ? editorColors[
                      ...
                      labels: wakapiData.editors
                          .slice(
                              0,
                              Math.min(showTopN[2], wakapiData.editors.length)
                          )
                          .map((p) => p.key),
                  },
                  options: {
            ...
            ? new Chart(languagesCanvas.getContext('2d'), {
                  ...
                  data: {
                      datasets: [
                          {
                              ...
                              hoverBackgroundColor: wakapiData.languages.map(
                                  (p) => {
                                      const c = hexToRgb(
                                          languageColors[p.key.toLowerCase()] ||
                                              getRandomColor(p.key)
                      ...
                      labels: wakapiData.languages
                          .slice(
                              0,
                              Math.min(showTopN[3], wakapiData.languages.length)
                          )
                          .map((p) => p.key),
                  },
                  options: {
            ...
            ? new Chart(machinesCanvas.getContext('2d'), {
                  ...
                  data: {
                      datasets: [
                          {
                              data: wakapiData.machines
                                  .slice(
                                      ...
                                      Math.min(
                                          showTopN[4],
                                          wakapiData.machines.length
                                      )
                                  )
                                  .map((p) => parseInt(p.total)),
                              backgroundColor: wakapiData.machines.map(
                                  (p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                                              ? getRandomColor(p.key)
                              ...
                              hoverBackgroundColor: wakapiData.machines.map(
                                  (p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                                              ? getRandomColor(p.key)
                      ...
                      labels: wakapiData.machines
                          .slice(
                              0,
                              Math.min(showTopN[4], wakapiData.machines.length)
                          )
                          .map((p) => p.key),
                  },
                  options: {
            ...
            ? new Chart(labelsCanvas.getContext('2d'), {
                  ...
                  data: {
                      datasets: [
                          {
                              ...
                              hoverBackgroundColor: wakapiData.labels.map(
                                  (p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                                              ? getRandomColor(p.key)
                      ...
                      labels: wakapiData.labels
                          .slice(
                              0,
                              Math.min(showTopN[5], wakapiData.labels.length)
                          )
                          .map((p) => p.key),
                  },
                  options: {
            ...
            ? new Chart(branchesCanvas.getContext('2d'), {
                  ...
                  data: {
                      datasets: [
                          {
                              data: wakapiData.branches
                                  .slice(
                                      ...
                                      Math.min(
                                          showTopN[6],
                                          wakapiData.branches.length
                                      )
                                  )
                                  .map((p) => parseInt(p.total)),
                              backgroundColor: wakapiData.branches.map(
                                  (p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                                              ? getRandomColor(p.key)
                              ...
                              hoverBackgroundColor: wakapiData.branches.map(
                                  (p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                                              ? getRandomColor(p.key)
                      ...
                      labels: wakapiData.branches
                          .slice(
                              0,
                              Math.min(showTopN[6], wakapiData.branches.length)
                          )
                          .map((p) => p.key),
                  },
                  options: {
            ...
            ? new Chart(entitiesCanvas.getContext('2d'), {
                  ...
                  data: {
                      datasets: [
                          {
                              data: wakapiData.entities
                                  .slice(
                                      ...
                                      Math.min(
                                          showTopN[7],
                                          wakapiData.entities.length
                                      )
                                  )
                                  .map((p) => parseInt(p.total)),
                              backgroundColor: wakapiData.entities.map(
                                  (p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                                              ? getRandomColor(p.key)
                              ...
                              hoverBackgroundColor: wakapiData.entities.map(
                                  (p, i) => {
                                      const c = hexToRgb(
                                          vibrantColors
                                              ? getRandomColor(p.key)
                      ...
                      labels: wakapiData.entities
                          .slice(
                              0,
                              Math.min(showTopN[7], wakapiData.entities.length)
                          )
